  ...
  useEffect(() => {
    const fetchSolarPulse = async () => {
      try {
        ...
        setSolarData(data[1]?.kp?.[0] || 2);
      } catch { setSolarData(2.5); }
      finally { setIsLoading(false); }
    };
    fetchSolarPulse();
    const int1 = setInterval(fetchSolarPulse, 60000);
      ...
      <ScrollView style={styles.content} showsVerticalScrollIndicator={false}>
        ...
        {activeTab === 1 && (
          <View>
            ...
            <View style={[styles.card, { borderLeftColor: localData.color }]}>
              ...
              <Text style={styles.mono}>X: {localData.x.toFixed(2)} | Y: {localData.y.toFixed(2)} | Z: {localData.z.toFixed(2)} µT</Text>
            </View>
            <View style={styles.synthesisCard}>
              <Text style={styles.cardTitle}>Diagnóstico de Acoplamiento</Text>
              <Text style={styles.synthesisText}>{solarData > 4 ? "⚠️ El entorno cósmico inyecta energía (Xi). Su biología aumentará la necesidad de disipar (Ji)." : "✅ Campo geomagnético estable. Estructura disipativa operando en su punto óptimo."}</Text>
            </View>
          </View>
        )}
        ...
        {activeTab === 5 && (
          <View>
            ...
            <Text style={styles.h2} style={{textAlign:'center', color:'#38bdf8'}}>CHRONOSFERA: El Pulso de la Noósfera</Text>
            <Text style={styles.bodyText} style={{textAlign:'center', fontStyle:'italic', marginBottom:20}}>Monitor de Acoplamiento Solar y Carga Disipativa para la Validación Ciudadana de la Vida como Imperativo Entrópico.</Text>
            ...
            <View style={styles.synthesisCard}>
              <Text style={styles.h2} style={{color:'#38bdf8'}}>Dirección Científica del Proyecto</Text>
